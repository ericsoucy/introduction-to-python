...
# Difference Between Prefixing Attributes with Single and Double Underscore: Prefixing attributes and methods with a single underscore means they are meant for internal use. This is a convention, and it doesn't enforce accessing attributes from the outside. Prefixing attributes 
...
# Single and Multiple Inheritance: When a child class inherits properties and methods from a single parent,
# as you can see above, the process is called single inheritance. 
# When a child class inherits properties and methods from more than one parent, that is multiple inheritance.
...
# but each class implements it differently when called. Here's the syntax for it:
class A:
   def action(self): ...
...
# The Purpose of Name Mangling: The main purpose of name mangling is to prevent accidental attribute 
# and method overriding when you use inheritance. Here's a code that makes that more understandable:
class Parent:
    def __init__(self):
...
# How Python Implements Abstraction: Python implements abstraction through the abc module.
# The module provides the ABC class (abstract base class) and the @abstractmethod decorator.
# An abstract base class (ABC) defines the common methods and properties subclasses must implement. It can't be instantiated.
# How Abstract Method is Defined: An abstract method is defined with @abstractmethod and must be overridden in subclasses, even if it has a default implementation.
